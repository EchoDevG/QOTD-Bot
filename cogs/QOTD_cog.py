...
class QOTD(commands.Cog):
    ...
    async def called_once_a_day(self, client):  # Fired every day
        ...
        try:
            if os.stat("/home/container/text/questionQueue.txt").st_size > 0:
                print("Looking in question queue")
                outputMessage = await self.check_queue()
    ...
    async def background_task(self, client):
        ...
        while True:
            ...
            target_time = datetime.combine(now.date(), self.WHEN)  # 6:00 PM today (In UTC)
            seconds_until_target = (target_time - now).total_seconds()
            await asyncio.sleep(seconds_until_target)  # Sleep until we hit the target time
            await self.called_once_a_day(client)  # Call the helper function that sends the message
            tomorrow = datetime.combine(now.date() + timedelta(days=1), time(0))
            seconds = (tomorrow - now).total_seconds()  # Seconds until tomorrow (midnight)
            await asyncio.sleep(
                seconds
    ...
    @commands.command(description="Add questions to the bot")
